...
async function fetchKeys() {
  ...
  try {
    ...
    toast.add({ severity: 'error', summary: t('common.error'), detail: t('cdKeys.failedToLoad'), life: 3000 })
  } finally {
    loading.value = false
  }
}
...
async function openEditDialog(cdKey: CdKey) {
  ...
  try {
    ...
    toast.add({ severity: 'error', summary: t('common.error'), detail: t('cdKeys.failedToLoadDetail'), life: 3000 })
  }
}

async function saveKey() {
  if (!keyForm.value.key.trim()) {
    toast.add({ severity: 'warn', summary: t('common.validation'), detail: t('cdKeys.keyCodeRequired'), life: 3000 })
    return
  }
  ...
  try {
    ...
    if (keyDialogMode.value === 'create') {
      ...
      toast.add({ severity: 'success', summary: t('common.success'), detail: t('cdKeys.keyCreated', { key: data.key }), life: 3000 })
    } else {
      await updateCdKey(editingKeyId.value!, { ...data, maxRedeemCount: data.maxRedeemCount })
      toast.add({ severity: 'success', summary: t('common.success'), detail: t('cdKeys.keyUpdated', { key: data.key }), life: 3000 })
    }

    showKeyDialog.value = false
    fetchKeys()
  } catch (err: any) {
    const msg = err?.response?.data?.message || t('cdKeys.failedToSave')
...
function confirmDeleteKey(cdKey: CdKey) {
  confirmService.require({
    message: t('cdKeys.confirmDeleteMessage', { key: cdKey.key }),
    ...
    accept: async () => {
      try {
        ...
        toast.add({ severity: 'success', summary: t('common.success'), detail: t('cdKeys.keyDeleted', { key: cdKey.key }), life: 3000 })
        fetchKeys()
      } catch {
        toast.add({ severity: 'error', summary: t('common.error'), detail: t('cdKeys.failedToDelete'), life: 3000 })
      }
    },
  })
}
...
async function executeRedeem() {
  ...
  try {
    ...
    toast.add({ severity: 'success', summary: t('cdKeys.redeemSuccess'), detail: result.message, life: 4000 })
    showRedeemDialog.value = false
    fetchKeys() // refresh counts
  } catch (err: any) {
    const msg = err?.response?.data?.message || t('cdKeys.failedToRedeem')
...
function formatDate(dateStr: string | null): string {
  if (!dateStr) return t('common.noExpiry')
...
<template>
  <div class="cdkeys-view">
    ...
    <!-- Sub-tab navigation -->
    <div class="sub-tabs">
      <button class="sub-tab sub-tab--active">{{ t('cdKeys.keys') }}</button>
      <button class="sub-tab" @click="navigateTo('redemptions')">{{ t('cdKeys.redemptions') }}</button>
    </div>

    <div class="toolbar">
      <span class="p-input-icon-left search-wrapper">
        <i class="pi pi-search" />
        <InputText v-model="searchFilter" :placeholder="t('cdKeys.searchPlaceholder')" class="search-input" @input="onSearch" />
      </span>
      <Button icon="pi pi-refresh" text severity="secondary" @click="fetchKeys" :loading="loading" />
      <div class="toolbar-spacer" />
      <Button
        v-if="canManageCdKeys"
        :label="t('cdKeys.addCdKey')"
        icon="pi pi-plus"
        severity="info"
        size="small"
    ...
    <DataTable
      ...
      @page="onPage"
    >
      <Column field="key" :header="t('cdKeys.keyCode')">
        <template #body="{ data }">
          <code class="key-text">{{ data.key }}</code>
        </template>
      </Column>

      <Column :header="t('cdKeys.redeemed')" style="width: 130px">
        <template #body="{ data }">
          <span>{{ redeemCounts[data.id] ?? '...' }} / {{ data.maxRedeemCount }}</span>
        </template>
      </Column>

      <Column field="expiryAt" :header="t('cdKeys.expiry')" style="width: 200px">
        <template #body="{ data }">
          <Tag v-if="isExpired(data.expiryAt)" :value="t('common.expired')" severity="danger" />
          <span v-else-if="data.expiryAt" class="date-text">{{ formatDate(data.expiryAt) }}</span>
          <span v-else class="empty-text">{{ t('common.noExpiry') }}</span>
        </template>
      </Column>

      <Column field="description" :header="t('cdKeys.description')">
        <template #body="{ data }">
          <span class="desc-text">{{ data.description || '—' }}</span>
        </template>
      </Column>

      <Column :header="t('cdKeys.actions')" style="width: 160px">
        <template #body="{ data }">
          <div class="action-buttons">
            <Button
              ...
              :title="t('cdKeys.redeemForPlayer')"
            />
            <Button
              v-if="canManageCdKeys"
              icon="pi pi-pencil"
      ...
      <template #empty>
        <div class="empty-state">
          ...
          <p>{{ t('cdKeys.noCdKeysYet') }}</p>
          <span class="empty-hint">{{ t('cdKeys.createCodesHint') }}</span>
        </div>
      </template>
    </DataTable>
    ...
    <Dialog
      ...
      :header="keyDialogMode === 'create' ? t('cdKeys.createCdKey') : t('cdKeys.editCdKey')"
      :modal="true"
      :style="{ width: '550px' }"
    >
      <div class="form-grid">
        <div class="form-field">
          <label>{{ t('cdKeys.keyCodeLabel') }}</label>
          <InputText v-model="keyForm.key" :placeholder="t('cdKeys.keyCodePlaceholder')" class="w-full" />
        </div>

        <div class="form-row">
          <div class="form-field">
            <label>{{ t('cdKeys.maxRedemptions') }}</label>
            <InputNumber v-model="keyForm.maxRedeemCount" :min="1" :max="999999" showButtons class="w-full" />
          </div>
          <div class="form-field">
            <label>{{ t('cdKeys.expiryDate') }}</label>
            <DatePicker v-model="keyForm.expiryAt" showTime hourFormat="24" showIcon class="w-full" :placeholder="t('cdKeys.expiryPlaceholder')" />
          </div>
        </div>

        <div class="form-field">
          <label>{{ t('cdKeys.descriptionLabel') }}</label>
          <Textarea v-model="keyForm.description" rows="2" class="w-full" :placeholder="t('cdKeys.descriptionPlaceholder')" />
        </div>

        <div class="form-field">
          <label>{{ t('cdKeys.linkedItems') }}</label>
          <MultiSelect
            v-model="keyForm.itemIds"
            :options="itemDefs"
            optionLabel="itemName"
            optionValue="id"
            :placeholder="t('cdKeys.selectItems')"
            class="w-full"
            filter
            display="chip"
          />
        ...
        <div class="form-field">
          <label>{{ t('cdKeys.linkedCommands') }}</label>
          <MultiSelect
            v-model="keyForm.commandIds"
            :options="cmdDefs"
            optionLabel="command"
            optionValue="id"
            :placeholder="t('cdKeys.selectCommands')"
            class="w-full"
            filter
            display="chip"
          />
      ...
      <template #footer>
        <Button :label="t('common.cancel')" text severity="secondary" @click="showKeyDialog = false" />
        <Button
          :label="keyDialogMode === 'create' ? t('common.create') : t('common.save')"
          severity="info"
          @click="saveKey"
    ...
    <Dialog
      ...
      :header="t('cdKeys.redeemCdKey')"
      :modal="true"
      :style="{ width: '400px' }"
    >
      <div class="redeem-form" v-if="redeemKey">
        <p class="redeem-info">
          {{ t('cdKeys.key') }}: <code class="key-text">{{ redeemKey.key }}</code>
          <br />
          {{ t('cdKeys.redeemed') }}: {{ redeemCounts[redeemKey.id] ?? '...' }} / {{ redeemKey.maxRedeemCount }}
          <br />
          <span v-if="redeemKey.expiryAt">
            {{ t('cdKeys.expiry') }}:
            <Tag v-if="isExpired(redeemKey.expiryAt)" :value="t('common.expired')" severity="danger" />
            <span v-else>{{ formatDate(redeemKey.expiryAt) }}</span>
          </span>
          <span v-else>{{ t('cdKeys.expiry') }}: <span class="empty-text">{{ t('common.none') }}</span></span>
        </p>

        <div class="form-field">
          <label>{{ t('cdKeys.selectPlayerOnline') }}</label>
          <Select
            v-model="redeemPlayerId"
            :options="onlinePlayers()"
            optionLabel="playerName"
            optionValue="playerId"
            :placeholder="t('cdKeys.choosePlayer')"
            class="w-full"
            filter
          />
        </div>
      ...
      <template #footer>
        <Button :label="t('common.cancel')" text severity="secondary" @click="showRedeemDialog = false" />
        <Button
          :label="t('cdKeys.redeem')"
          icon="pi pi-gift"
          severity="info"
          @click="executeRedeem"
